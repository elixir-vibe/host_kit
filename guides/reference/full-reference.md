...
Run records are intentionally compact: they identify the collision-resistant run id, project, direction, timestamp, and applied change statuses. They do not replace plan artifacts; use plan artifacts for inspectable up/down plan contents. Run records and copied payloads are published atomically with restrictive file permissions, and corrupt records are reported instead of silently skipped. When a tracked apply is started from `--plan`, HostKit copies that up-plan artifact under the runs root and records the copied path so `mix host_kit.down --last` can work from the tracked run.
...
Source updates are intentionally not inferred as reversible by default: a previous Git remote/ref may no longer be reachable. Treat source rollback as an explicit lifecycle operation or pair it with a backup/source-bundle strategy. Git source operations run as the target connection user by default, even during `--sudo` applies, so SSH credentials and known-hosts remain in the operator context. Use `source ..., sudo: true` only for checkouts that intentionally require root-owned Git operations.
...
HostKit can run as a supervised OTP application. The supervision tree starts agent state plus idle drift and monitor workers; callers can run checks explicitly through the runtime API:
...
HostKit has a Dotenvy-validated `dotenv` resource for explicit env files. Secret values are resolved at apply time. Drift detection compares metadata and non-secret `set` entries with structured key-level diffs; secret entry values are not read into plan artifacts for comparison. Use `secret KEY, env: :redacted` for existing/generated env-file secrets that should be modeled without exposing their values. When updating an existing dotenv file, HostKit reads those values only at the apply boundary and preserves them while rendering changed public entries; modeled optional keys that are already absent remain absent. A redacted secret still cannot create a missing file. Secret sources support `env: "NAME"`, `file: "/run/secrets/name"`, and `command: ["pass", "show", "name"]`.
...
mix host_kit.clean --require toys_infra.exs infra/config.exs --local --service my_app --keep 2 --dry-run
