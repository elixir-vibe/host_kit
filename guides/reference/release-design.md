...
During `host_kit.apply`, HostKit first collects these ReleaseKit build specs and builds a normal preparation plan from existing HostKit resources. That preparation plan includes selected `source` resources and generated `command` resources that run `mix deps.get --only MIX_ENV`, clean generated Mix release staging, and run `mix release_kit.artifact` through the HostKit runner boundary. Build commands declare source inputs, path inputs, outputs, and stamps, so source checkout and artifact preparation are ordered and reported through the same plan/apply machinery as other resources. When `release_kit: [user: ...]` is configured, these preparation commands are modeled with `Command.user` instead of embedding ad-hoc `sudo` in the command string; execution runs as that user while reproducibility stamps are written through the normal apply privilege boundary. HostKit then reloads the project and consumes the generated manifest normally for the deployment plan. Dry runs do not build artifacts. Plain `manifest:` remains supported for workflows that build artifacts outside HostKit.

Release declarations also record release metadata on the existing service. `mix host_kit.clean` uses that metadata to build explicit cleanup command plans for inactive versions. Automatic retention follows immutable release-directory modification time, not lexical version order. Operators can repeat `--protect-version VERSION` to preserve verified rollback releases when failed deployments created newer unusable directories. This keeps cleanup out of normal convergence plans while still using inspectable HostKit plan/apply data.
